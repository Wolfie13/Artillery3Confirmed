...
public sealed class Projectile : MonoBehaviour 
{
    ...
    [SerializeField, Range (0f, 50f)]   private float m_range          = 10f;   //!< The area of effect for the projectile.
    [SerializeField, Range (0f, 50f)]   private float m_maxDamageRange = 0.5f;  //!< The range at which maximum, unscaled damage will be applied.
    ...
    {
        // Don't waste computation time unnecessarily.
        if (m_range > 0f)
        {
            ApplyDamage();
        }

        // Finally spawn any children, special effects and destroy thyself.
        SpawnProjectiles();
        SpecialEffects();
        Destroy (gameObject);
    }
    ...
    private void SpawnProjectiles()
    {
        ...
        if (m_spawnOnDetonation && m_spawnCount > 0)
        {
            ...
            {
                // Determine a random force scalar to be applied and calculate the starting position.
                Vector3 unscaledForce = Random.insideUnitCircle;
                Vector3 position      = rigidbody.position + unscaledForce * 0.2f;

                // Instantiate the object and update its velocity.
                Projectile child = Instantiate (m_spawnOnDetonation, position, Random.rotation) as Projectile;
